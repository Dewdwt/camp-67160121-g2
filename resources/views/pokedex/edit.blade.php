...
@section('page-title', 'แก้ไขข้อมูล ' . $pokedex->name)
@section('container-class', 'container-large')

@section('content')
<div class="card border-0 shadow">
    <div class="card-header bg-warning text-dark">
        <h5 class="mb-0"><i class="fas fa-edit me-2"></i>แก้ไขข้อมูล {{ $pokedex->name }}</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('pokedex.update', $pokedex) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                           id="name" name="name" value="{{ old('name', $pokedex->name) }}" required>
                    @error('name')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                        ...
                        <option value="Fire" {{ old('type', $pokedex->type) == 'Fire' ? 'selected' : '' }}>Fire</option>
                        <option value="Water" {{ old('type', $pokedex->type) == 'Water' ? 'selected' : '' }}>Water</option>
                        <option value="Grass" {{ old('type', $pokedex->type) == 'Grass' ? 'selected' : '' }}>Grass</option>
                        <option value="Electric" {{ old('type', $pokedex->type) == 'Electric' ? 'selected' : '' }}>Electric</option>
                        <option value="Bug" {{ old('type', $pokedex->type) == 'Bug' ? 'selected' : '' }}>Bug</option>
                        <option value="Normal" {{ old('type', $pokedex->type) == 'Normal' ? 'selected' : '' }}>Normal</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('species') is-invalid @enderror"
                       id="species" name="species" value="{{ old('species', $pokedex->species) }}" required>
                @error('species')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" step="0.01" min="0" class="form-control @error('height') is-invalid @enderror"
                           id="height" name="height" value="{{ old('height', $pokedex->height) }}" required>
                    @error('height')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" step="0.1" min="0" class="form-control @error('weight') is-invalid @enderror"
                           id="weight" name="weight" value="{{ old('weight', $pokedex->weight) }}" required>
                    @error('weight')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" min="1" max="500" class="form-control @error('hp') is-invalid @enderror"
                           id="hp" name="hp" value="{{ old('hp', $pokedex->hp) }}" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" min="1" max="300" class="form-control @error('attack') is-invalid @enderror"
                           id="attack" name="attack" value="{{ old('attack', $pokedex->attack) }}" required>
                    @error('attack')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" min="1" max="300" class="form-control @error('defense') is-invalid @enderror"
                           id="defense" name="defense" value="{{ old('defense', $pokedex->defense) }}" required>
                    @error('defense')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                ...
                <input type="url" class="form-control @error('image_url') is-invalid @enderror"
                       id="image_url" name="image_url" value="{{ old('image_url', $pokedex->image_url) }}"
